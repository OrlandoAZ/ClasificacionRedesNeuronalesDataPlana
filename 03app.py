...
NOTA IMPORTANTE SOBRE EL FORMATO DEL MODELO:
El modelo ya NO se carga con `tf.keras.models.load_model(...)` sobre un
archivo `.keras`. Ese formato nativo de Keras 3 construye las rutas
internas de cada capa con `tf.io.gfile.join(...)`, que en Windows usa
`os.path.join` (separador `\`) y en Linux usa `/`. Esto hace que un
`.keras` guardado en Windows falle al cargarse en Streamlit Cloud (Linux),
aunque cargue perfecto en local.

En vez de eso, aquí se reconstruye la arquitectura en código (idéntica a
la del notebook) y se cargan solo los PESOS desde un `.h5` en formato
"legacy" (el clásico de Keras 2, anterior al formato nativo `.keras`).
Ese formato legacy identifica cada capa por su `layer.name` directamente
como grupo HDF5, sin usar `gfile.join` en ningún punto - por eso es
idéntico en Windows, Linux y macOS.

Archivos requeridos en el mismo directorio que este script:
    - modelo_MLP_churn_pesos.h5   (pesos en formato legacy, NO terminar en .weights.h5)
...
MODEL_WEIGHTS_PATH = "modelo_MLP_churn_pesos.h5"
PREPROC_PATH = "preprocesamiento_churn.pkl"

N_FEATURES = 11  # 6 numericas + 5 dummies, igual que en el notebook
...
@st.cache_resource
def cargar_modelo():
    modelo = construir_arquitectura()
    modelo.load_weights(MODEL_WEIGHTS_PATH)
    return modelo
...
try:
    ...
    st.error(
        "No se encontraron los archivos del modelo. Asegurate de subir "
        f"`{MODEL_WEIGHTS_PATH}` y `{PREPROC_PATH}` al mismo directorio que `03app.py`.\n\n"
        f"Detalle: {e}"
    )
    st.stop()
